...
| 仅基于 PNG origW 判断 Mermaid | mmdc -w 3600 强制所有 Mermaid 图 origW=3600，无法区分 |
...
v4 中的以下内容被本方案修改：
- 横置触发条件：普通图片从 `aspectRatio > 1.5` 改为 `downscaleRatio > 3 && aspectRatio > 2.0`；Mermaid 改为基于 SVG viewBox 自然宽度
- 触发范围：两种图片各自有独立的判断逻辑
